...
async def test_async_client_fetch_manifest():
    ...
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        ...
        # Test with URL
        manifest = await fetch_manifest("https://example.com/.well-known/ai-catalog.json")
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        ...
        async with AgentFinderClient(base_url="https://registry.example.com/api/v1") as client:
            response = await client.search(
                ...
                filter_={"type": ["application/a2a-agent-card+json"], "compliance": "hipaa"},
            ...
            assert kwargs["json"]["query"]["filter"]["type"] == ["application/a2a-agent-card+json"]
...
async def test_async_client_explore():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        ...
        async with AgentFinderClient(base_url="https://registry.example.com/api/v1") as client:
            result_type = ExploreResultType(facets=[ExploreFacetRequest(field="type", limit=10)])
            ...
            assert response.facets["type"].buckets[0].value == "application/mcp-server+json"
            ...
            assert kwargs["json"]["query"]["filter"]["trustManifest.attestations.type"] == ["SOC2-Type2"]
...
async def test_client_from_manifest():
    ...
    client2 = AgentFinderClient.from_manifest(manifest, "urn:ai:example.com:registry:global")
    assert client2.base_url == "https://registry.example.com/api/v1"

    # 3. Error raised when identifier is not found
    with pytest.raises(ValueError) as exc_info:
        AgentFinderClient.from_manifest(manifest, "urn:ai:example.com:registry:not-exist")
...
async def test_client_custom_client_configuration():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        ...
        async with httpx.AsyncClient(headers=custom_headers, timeout=custom_timeout) as async_client:
            ...
            async with AgentFinderClient(base_url="https://registry.example.com/api/v1", client=async_client) as client:
                ...
                mock_post.assert_called_once()
                assert mock_post.call_args[0][0] == "https://registry.example.com/api/v1/search"
